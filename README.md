...
- 10개 생활 분야 탐색
- 페이지 단위 추가 로딩
- 신청 상태를 `상시 신청`, `마감 임박`, `모집 종료`, `모집기간 확인 필요`로 정규화
- 오래된 공고는 카드에서 모집 종료 상태와 다음 행동을 분리해 표시
- 지역명·분야명이 원문과 충돌할 때 제목·기관 정보를 함께 확인해 표시값을 보정
...
- JDBC 기반 서버 세션
- 관심 혜택 브라우저 저장
- 비교 목록을 브라우저에 저장하고 최대 3개 혜택을 지원대상·지원내용·신청기간·신청방식 기준으로 나란히 비교
- 서버 기준으로 전체 혜택에 적용되는 마감 임박, 온라인 안내, 내 조건 일치 빠른 보기
- 결과 화면에서 현재 탐색 조건을 칩으로 확인하고 추천순·신청기간순·온라인 안내순으로 정렬
- 조건 초기화로 맞춤진단과 수동 검색을 한 번에 비우고 전체 혜택 탐색으로 복귀
- 데이터 동기화 상태와 마지막 기준 시각을 목록 상단에서 확인
- 로그인 계정에 맞춤 조건 저장 및 기기 간 복원
- 사용자별 조건·서류 체크와 진행상태 저장
- 최대 3개 혜택 비교 결과를 로그인 없이 링크로 공유
- 상세 화면에서 개인정보 없이 공고 오류·기간·대상·공식 링크 문제 제보
...
- 신청 방식에 따라 `신청 준비`, `이용 방법 확인`, `공식 안내 확인` CTA를 구분
...
안심생활은 혜택을 많이 나열하는 검색 결과가 아니라, 사용자가 실제로 다음 행동을 결정하는 `혜택 발견 OS`를 지향합니다. 토스의 개인화·직접 비교 경험, 당근의 가까운 생활 맥락, 번개장터의 탐색·필터 구조에서 영감을 얻되 공공혜택 서비스에 맞게 재구성했습니다. 핵심은 발견 화면과 실행 화면을 분리하지 않는 것입니다. 목록에서는 현재 조건과 추천 우선순위를 명확히 보여주고, 상세 화면에서는 공식 기준·서류·접수처를 확인하며, 준비실에서는 사용자의 체크와 외부 접수 이후 기록까지 이어지도록 구성했습니다.

현재 화면은 다음 네 가지 질문에 답하도록 설계했습니다.

1. `무엇을 보고 있나요?` — 현재 지역·분야·검색어·맞춤 조건을 결과 상단에 표시합니다.
2. `무엇부터 보나요?` — 추천순, 신청기간순, 온라인 안내순으로 목록의 목적을 바꿀 수 있습니다.
3. `왜 이 혜택인가요?` — 내 조건과 명시적으로 일치하는지 또는 공식 기준을 더 확인해야 하는지 구분합니다.
4. `다음 행동은 무엇인가요?` — 비교, 상세 확인, 신청 준비, 공식 기관 접수, 결과 기록으로 이어집니다.

빠른 보기는 서버에 `filter=urgent|online|matched` 조건을 전달해 저장된 전체 혜택을 기준으로 계산합니다. `혜택 더 보기`로 다음 페이지를 불러와도 같은 조건이 유지됩니다. 목록의 신청 상태는 원문 기간을 기계적으로 읽은 표시값이며, 최종 자격·모집 여부·접수 가능 여부는 공식 공고와 담당기관이 확정합니다.

### 수익화로 이어지는 제품 전략

수익화는 공공혜택 정보를 먼저 유료화하는 방식으로 시작하지 않습니다. 무료 핵심 기능으로 신뢰와 반복 사용을 만들고, 그 위에 사용자의 시간을 실제로 줄여주는 부가가치를 단계적으로 얹는 전략입니다.

```mermaid
flowchart LR
    A[공식 혜택 발견] --> B[비교·상세 확인]
    B --> C[신청 준비·진행 기록]
    B --> D[비교 링크 공유]
    D --> A
    B --> E[정보 오류 제보]
...
현재 단계에서 구현한 수익화 기반은 두 가지입니다.

1. `공유 가능한 비교 결과`: 사용자가 가족·상담사·지인에게 같은 혜택 묶음을 전달할 수 있어 자연스러운 신규 유입 경로가 됩니다. 로그인이나 개인정보 입력 없이 동작합니다.
2. `정보 품질 제보`: 잘못된 기간·대상·링크를 발견한 사용자가 바로 알려주고, 혜택 ID와 제보 유형만 서버에 남겨 운영 품질을 개선합니다. 품질이 쌓여야 알림·기관용 도구와 같은 유료 가치도 정직하게 만들 수 있습니다.

확장 순서는 다음과 같이 잡습니다.

| 단계 | 사용자 가치 | 수익화와의 연결 | 현재 상태 |
| --- | --- | --- | --- |
| 1. 신뢰 기반 | 공식 출처, 상세 기준, 비교, 준비실, 오류 제보 | 서비스의 핵심 신뢰와 데이터 자산 형성 | 구현 완료 |
| 2. 재방문 | 마감·공고 변경·다음 확인일 알림, 계정 간 동기화 | 알림 묶음·고급 준비실 같은 선택형 편의 기능 검토 | 다음 단계 |
| 3. 운영 품질 | 관리자 검수 큐, 링크 상태 점검, 변경 이력, 제보 처리 SLA | 기관·상담기관용 품질 대시보드 검토 | 이후 |
| 4. 제휴 실험 | 공식 승인을 받은 안내·예약·상담 연결 | 이용자에게 명확히 표시한 제휴 수수료 또는 기관용 구독 | 승인 후 |

기본 공공혜택 검색과 공식 기준 확인을 돈으로 막지 않고, 광고·제휴가 추천 순위나 자격 판단을 바꾸지 않도록 운영할 예정입니다. 아직 결제·광고·제휴 API는 연결하지 않았으며, 실제 상품·약관·파트너 승인이 정해진 뒤 별도 검토합니다.
...
│  │  ├─ BenefitDeadline.java
│  │  ├─ BenefitPresentation.java
...
```json
{
  "programId": 1,
  "type": "WRONG_DEADLINE",
  "message": "공식 공고의 마감일과 달라요."
}
```

`type`은 `OUTDATED`, `WRONG_TARGET`, `WRONG_DEADLINE`, `BROKEN_LINK`, `OTHER` 중 하나입니다. 로그인은 필요하지 않지만 혜택 ID가 실제로 존재해야 하며, 내용은 5~500자로 제한합니다. 이 API는 이름·이메일·전화번호·IP를 저장하거나 반환하지 않고, `benefit_feedback` 테이블에 혜택 ID·유형·내용·접수시각만 저장합니다. 현재는 운영자 검수 화면과 자동 rate limit이 없으므로 공개 규모를 키우기 전에 인증된 관리자 큐, 중복 제보 병합, 스팸 제한, 처리 상태 이력을 추가해야 합니다.
...
혜택 정보의 오류나 변경 가능성을 운영 큐로 전달합니다. `programId`로 `SupportProgram`을 가리키며, 제보자 개인정보나 신청서 데이터는 저장하지 않습니다. 유형은 오래된 공고, 대상 오류, 기간 오류, 링크 오류, 기타로 제한합니다.
...
- 정보 품질 제보 API가 혜택 존재 여부와 내용 길이를 서버에서 검증함
...
- 회원탈퇴 및 개인정보 삭제
- 관리자 접근통제와 감사 로그
- 품질 제보 관리자 검수 큐와 공개 제보 rate limit
- 애플리케이션 암호화 키 관리
...
1. 맞춤진단·검색 정확도와 혜택 정보 품질 제보 처리 개선
2. 이메일 인증, 탈퇴, 데이터 삭제와 계정 보안 보강
3. 공고 변경·마감·다음 확인일 알림
4. 관리자 검수 큐, 링크 상태 점검, 정보 변경 이력
5. 지역·대상별 신청 가이드 품질 개선
6. 이용자에게 무료 핵심 검색을 유지한 선택형 프리미엄 편의 기능 검토
7. 기관과 정식 협의가 완료된 제휴·품질 대시보드 파일럿 검토
