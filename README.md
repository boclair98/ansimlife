...
비교·빠른 보기·정렬은 현재 화면에 불러온 목록을 기준으로 동작합니다. `혜택 더 보기`로 다음 목록을 불러오면 같은 선택 필터와 정렬이 이어서 적용되며, 최종 자격은 비교 결과가 아니라 공식 공고와 담당기관이 확정합니다. 다음 고도화 단계에서는 서버 검색 결과에도 동일한 정렬·추천 규칙을 적용해 페이지가 바뀌어도 우선순위가 흔들리지 않도록 할 예정입니다.
